<template>
  <div id="app">
    <right-nav :version="APP_VERSION" :settings="settings" :show-menu.sync="showMenu" :mode="mode" @modeChanged = "changeMode($event)"></right-nav>
    ...
    <div id="buttons">
      ...
      <el-select v-if="mode==='normal'" v-model="settings.selected_yaolings"  multiple collapse-tags style="margin-left: 20px;" placeholder="妖灵选择">
        <el-option  v-for="item in yaolings" :key="item.Id" :label="item.Name" :value="item.Id"></el-option>       
      </el-select>
    </div>
    <div id="qmap"></div>
  ...
  computed: {
    fit: function() {
      ...
      if (this.mode === 'normal') {
        ...
        ans = ans.concat(this.settings.selected_yaolings)
    ...
    progressPercent: function() {
      let maxInLine = this.settings.wide_range * 2 + 1;
